...
It teaches an agent to move from product intent to donor research, page strategy, section composition, UI system, responsive behavior, implementation handoff, and design review instead of stopping at decorative mockups.
...
`goal -> user scenario -> donor research -> page strategy -> section plan -> wireframe -> UI system -> responsive behavior -> implementation handoff -> review`
...
5. `blocks/design/READY_SITE_STACKS.md` when implementation stack matters
6. `blocks/design/LANDING_PAGE_PATTERNS.md` for landing pages
7. `blocks/design/SAAS_PATTERNS.md` for SaaS/product websites
8. `blocks/design/SECTION_LIBRARY.md` for page composition
9. `blocks/design/UI_COMPONENT_LIBRARY.md` for component/state planning
10. `blocks/design/DESIGN_SYSTEMS.md` for UI-system decisions
11. `blocks/design/MOTION_AND_ANIMATION.md` when motion is requested or visible
12. `blocks/design/CONVERSION_AND_MARKETING.md` when business conversion matters
13. `blocks/design/IMPLEMENTATION_HANDOFF.md` before frontend execution
14. `blocks/design/WEBSITE_REVIEW_CHECKLIST.md` for review
15. `blocks/design/DESIGN_REVIEW_STANDARD.md` when deeper formal review is needed
16. `blocks/design/DONORS.md` only when legacy donor rationale or older pattern notes are needed
